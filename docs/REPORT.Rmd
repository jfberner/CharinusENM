...
##### MaxEnt Map output
 ![Figure 1 = Output of MaxEnt model, as en example. From `04_Model-evaluation.R`](../figs/aocc/maxent_aocc_map.png)
 
##### MaxEnt Environmental output
 ![Figure 1 = Output of MaxEnt model, occurrence records and suitability in environmental space. From `04_Model-evaluation.R`](../figs/aocc/maxent_aocc_env.png)
...
To run the [accum.occ](https://github.com/LauraJim/SDM-hyperTest/blob/master/Accum_curve_occ.R)() function, the sdm and the predict object had to be manipulated alongside the occurrence records and the environment used in prediction. We built two functions to prepare the data (`R/sdm_to_occ.pnts.R` & `R/sdm_to_output.mod.R`), regarding the two objects required by the AOcC function. The outputs of [accum.occ](https://github.com/LauraJim/SDM-hyperTest/blob/master/Accum_curve_occ.R)() and [comp.accplot](https://github.com/LauraJim/SDM-hyperTest/blob/master/compare_models.R)() are in `figs/aocc/`.
 ...
 ![Figure 2 = accumulation of occurrences curves. A plot with the raster cells ranked from highest prediction to lowest in the x axis, and cummulative occurences in the y axis. The plot shows that the bioclim algorithm finds all occurrences in less cells than others, followed by max ent, then svm, and finally domain.](../figs/aocc/01_model-comparison.png)

We then selected the two best performing algorithms (SVM and MaxEnt) according to these two metrics to build a second round of models with 25 replicas for each algorithm `R/06_Final-Model`.

The evaluation of these final models (`R/07_Final-Model_Evaluation.R`) and their outputs and predictions are stored in `R/data/processed/final-model-build/`.
...
As a final step, we projected all layers along with the occurrence records on the following map (`R/10_leaflet-map.R`)
