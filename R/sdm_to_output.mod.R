...
#' 
#' @description This function takes the output from the sdm::predict() function, and environmental data used in predict() and makes a list of data.frames, one data.frame for each algorithm, in the output.mod format required by the accum.occ() function by Laura Jiménez (2020).
#'
...
#' 
#' d_occ <- sdmData(formula = charinus~., train=occ_train, predictors = envpres, bg = 18, method = 'eRandom') # Create sdmData object
#' 
