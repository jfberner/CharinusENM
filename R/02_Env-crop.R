...
>>>>>>> parent of 4fbc200 (Script updated to drop absence data in the M-defining buffer around occ records):R/01_Env-crop.R
...
raster::writeRaster(envlh, filename = 'data/processed/envcropped/LH/lh.tif', prj = T, overwrite = T, bylayer = T)

##### MH - Mid Holocene - 8.326 - 4.2ky
if (!dir.exists("data/processed/envcropped/MH")) dir.create("data/processed/envcropped/MH", recursive = TRUE)
raster::writeRaster(envmh, filename = 'data/processed/envcropped/MH/mh.tif', prj = T, overwrite = T, bylayer = T)

##### EH - Early Holocene - 11.7 - 8.326ky
if (!dir.exists("data/processed/envcropped/EH")) dir.create("data/processed/envcropped/EH", recursive = TRUE)
raster::writeRaster(enveh, filename = 'data/processed/envcropped/EH/eh.tif', prj = T, overwrite = T, bylayer = T)
